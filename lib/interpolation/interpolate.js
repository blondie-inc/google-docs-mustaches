...
const computeUpdates = async (placeholders, data, formatters, resolver) => {
    ...
    const replacements = await Promise.all(placeholders.map(async (placeholder) => {
        ...
        try {
            ...
            console.log("dot resolver", placeholder, computed);
            if (!computed && resolver) {
                console.log("interploate resolver", placeholder, computed);
            ...
            if (resolver) {
                computed = await resolver(placeholder);
                console.log("catch interploate resolver", placeholder, computed);
            }
        }
        return [placeholder, `${computed ? computed : ""}`];
    }));
    return replacements.map(([placeholder, computed]) => {
        console.log("123", placeholder, computed);
        const result = {
            replaceAllText: {
