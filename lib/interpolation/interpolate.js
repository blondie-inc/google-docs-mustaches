...
const computeUpdates = async (sections, placeholders, data, formatters, resolver) => {
    ...
    let mFunctionUpdates = await Promise.all(sections.map(async (section) => {
    ...
    let placeholderUpdates = await Promise.all(replacements.map(([placeholder, computed]) => {
        if (computed === placeholder)
            return;
        if (computed === "NaN")
            return;
        return {
