  ...
  await Promise.all(
    SPlaceholders.reverse().map(async (placeholder: SPlaceholderInfo) => {
      ...
      if (!sectionData && resolver) {
        console.log('[--- insertTableRowsQuery.ts:90 ---]', placeholder.placeholder);
        sectionData = await resolver(placeholder.placeholder);
        itemsLength = typeof sectionData.length === "function" ? sectionData.length() : sectionData.length;
      }

      console.log('[--- insertTableRowsQuery.ts:94 ---]', sectionData, itemsLength);
